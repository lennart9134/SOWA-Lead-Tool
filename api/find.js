...
// Gibt neue Leads zurück (ohne E-Mail/Telefon) — bereits in HubSpot vorhandene
// Kontakte (per linkedin_url) werden automatisch übersprungen, und es wird bei
...
import { findExistingLinkedinUrls } from "../lib/hubspot.js";
...
export default async function handler(req, res) {
  try {
    ...
    while (collected.length < limit && pages < MAX_PAGES) {
      ...
      try {
        existing = await findExistingLinkedinUrls(page.map((l) => l.linkedinUrl).filter(Boolean), token);
      } catch {
      ...
      for (const lead of page) {
        ...
        if (lead.linkedinUrl && existing.has(lead.linkedinUrl.toLowerCase())) continue; // schon in HubSpot
        seenKey.add(key);
        collected.push(lead);
